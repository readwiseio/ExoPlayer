...
import static com.readwise.android.exoplayer2.util.NalUnitUtil.NAL_START_CODE;
...
/* package */ final class RtspMediaTrack {
  ...
  public RtspMediaTrack(MediaDescription mediaDescription, Uri sessionUri) {
    checkArgument(
        mediaDescription.attributes.containsKey(SessionDescription.ATTR_CONTROL), "missing attribute control");
    payloadFormat = generatePayloadFormat(mediaDescription);
    uri = extractTrackUri(sessionUri, castNonNull(mediaDescription.attributes.get(
        SessionDescription.ATTR_CONTROL)));
  }
